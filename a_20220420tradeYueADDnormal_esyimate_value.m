function xout=a_20220420tradeYueADDnormal_esyimate_value(t,x)
%global kappa1;
%global kappa2;
%global kappa3;
global A;
...
delta1=x1-x3+x1-x0;
delta2=x2-x1+x2-x3;
x33=exp(A*timet2(i2)+0.05)*x3+i2*0.05*exp(-A*timet2(i2)/2)*B*UT3(i2);
x44=exp(A*timet2(i2)+0.05)*x4+i2*0.05*exp(-A*timet2(i2)/2)*B*UT4(i2);
x55=exp(A*timet2(i2)+0.05)*x5+i2*0.05*exp(-A*timet2(i2)/2)*B*UT5(i2);
if t>4
delta3=x3-x1+x3-x5;
end
...
%agent3in0-4s-agent4in0-8s/or4s-agent5.0-4sand0-8s
if t<=4
delta3=x3-x1+x3-x55;
end
if t<=4
...
timet2(i2+1)=t;UT3(i2+1)=u3;UT4(i2+1)=u4;UT5(i2+1)=u5;
...
i2=i2+1;
xout=[A*x1+B1*unew1;A*x2+B2*unew2;A*x3+B3*unew3;A*x4+B4*unew4;A*x5+B5*unew5;A*x0];
